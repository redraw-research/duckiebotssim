...
# Copy this into you dreamerv3 jax project
...
class UELaneFollowingEnv(gym.Env):
    ...
    def reset(self, episode_physics_parameters: Optional[EpisodicPhysicsParams] = None, **kwargs):
        self._step_counter = 0
        ...
        if self._randomize_physics_every_episode:
            if episode_physics_parameters:
                ...
                self._current_episode_physics_parameters = EpisodicPhysicsParams(
                    forward_mean=np.random.uniform(low=0.33, high=3.0),  # assume we may be off by up to 3x
                    forward_std=np.random.uniform(low=0.0, high=0.5),
                    yaw_mean=np.random.uniform(low=0.33, high=3.0),
                    yaw_std=np.random.uniform(low=0.0, high=0.5)
                )
...
class DreamerUELaneFollowingEnv(FromGym):
    ...
    def __init__(self, _, time_limit=200, physics_hz=None, use_mask=True,
        ...
        env = UELaneFollowingEnv(self._env_launch_config)
    ...
    def step(self, action):
        ...
        try:
            ...
            del self._env
            self._env = UELaneFollowingEnv(self._env_launch_config)
...
if __name__ == '__main__':
    ...
    # recommended DreamerUELaneFollowingEnv config for April 17 DR experiments
    # dreamer_env_config_kw = {"limit_backwards": False,
    #              "physics_hz": None,
    ...
    # env = DreamerUELaneFollowingEnv(None, **dreamer_env_config_kw)

    # recommended UELaneFollowingEnv config for April 17 DR experiments
    # (this should be equivalent to the DreamerUELaneFollowingEnv above)
    ue_env_config = {
        "use_domain_randomization": True,
        "randomize_mask": False,
        "render_game_on_screen": False,
    ...
    env = UELaneFollowingEnv(config=ue_env_config)

    print("env initialized")
    while True:
        env.reset(episode_physics_parameters=None)
        i = 0
