...
void URotateSensorCommand::Execute() {
	UE_LOG(LogDuckiebotsSim, Log, TEXT("URotateSensorCommand::Execute sensor"));

	if (StringParams.size() != 2 || NumberParams.size() != 3) {
		UE_LOG(LogDuckiebotsSim, Error, TEXT("Unexpected argument length found in URotateSensorCommand. Command not executed."));
		return;
	}
	...
	UE_LOG(LogDuckiebotsSim, Log, TEXT("roll %d pitch %d yaw %d"), RotationRoll, RotationPitch, RotationYaw);
