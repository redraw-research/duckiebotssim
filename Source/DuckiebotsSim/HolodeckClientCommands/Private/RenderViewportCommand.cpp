...
void URenderViewportCommand::Execute() {
	UE_LOG(LogDuckiebotsSim, Log, TEXT("URenderViewportCommand::Execute toggling viewport rendering"));

	if (StringParams.size() != 0 || NumberParams.size() != 1) {
		UE_LOG(LogDuckiebotsSim, Error, TEXT("Unexpected argument length found in URenderViewportCommand. Command not executed."));
		return;
	}
	...
	if (succeeded) {
		UE_LOG(LogDuckiebotsSim, Log, TEXT("URenderViewportCommand was successful"));
	} else {
		UE_LOG(LogDuckiebotsSim, Log, TEXT("URenderViewportCommand failed!"));
	}
