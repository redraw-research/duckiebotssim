...
UCommandCenter::UCommandCenter() {
	UE_LOG(LogDuckiebotsSim, Log, TEXT("CommandCenter::CommandCenter() constructed"));
}
...
void UCommandCenter::GetCommandBuffer() {
	UE_LOG(LogDuckiebotsSim, Log, TEXT("CommandCenter:: is getting command buffer"));
	if (Server == nullptr) {
		UE_LOG(LogDuckiebotsSim, Warning, TEXT("CommandCenter could not find server..."));
	} else {
		Buffer = static_cast<char*>(Server->Malloc(TCHAR_TO_UTF8(*BUFFER_NAME), BUFFER_SIZE));

		if (!Buffer) {
			UE_LOG(LogDuckiebotsSim, Fatal, TEXT("CommandCenter::GetCommandBuffer: Failed to allocate shared memory for buffer!"));
		}
		...
		if (ShouldReadBufferPtr != nullptr)
			*ShouldReadBufferPtr = false;
			...
			UE_LOG(LogDuckiebotsSim, Error, TEXT("UCommandCenter::ShouldReadBufferPtr is null"));
	}
}
...
int UCommandCenter::ReadCommandBuffer() {
	...
	if (Status != JSON_OK) {
		UE_LOG(LogDuckiebotsSim, Error, TEXT("Unable to parse command buffer as a json file"));
	} else {
...
void UCommandCenter::ExtractCommandsFromJson(const JsonValue &Input){
	if (Input.getTag() == JSON_OBJECT) {
		...
		if (Iter->value.getTag() == JSON_ARRAY)
			...
			UE_LOG(LogDuckiebotsSim, Warning, TEXT("Command Buffer didn't contain the format of JSON we expected. Unable to process command."));
	} else {
		UE_LOG(LogDuckiebotsSim, Warning, TEXT("Command Buffer didn't contain the format of JSON we expected. Unable to process command."));
	}
}
