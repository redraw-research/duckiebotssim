...
void USpawnAgentCommand::Execute() {
	...
	UE_LOG(LogDuckiebotsSim, Log, TEXT("SpawnAgentCommand::Execute spawning agent"));
	...
	if (GameTarget == nullptr) {
		UE_LOG(LogDuckiebotsSim, Warning, TEXT("UCommand::Target is not a UHolodeckGameMode*. SpawnAgentCommand::Execute Cannot spawn agent without this pointer!"));
		return;
	}

	UWorld* World = Target->GetWorld();
	if (World == nullptr) {
		UE_LOG(LogDuckiebotsSim, Warning, TEXT("SpawnAgentCommand::Execute found world as nullptr. Unable to spawn agent!"));
		return;
	}
	...
	UE_LOG(LogDuckiebotsSim, Log, TEXT("SpawnAgentCommand spawned a new Agent."));
