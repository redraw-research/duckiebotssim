...
void UCustomCommand::Execute() {
	UE_LOG(LogDuckiebotsSim, Log, TEXT("Executing CustomCommand."));
	if (StringParams.size() < 1) {
		UE_LOG(LogDuckiebotsSim, Error, TEXT("No command name found in CustomCommand call. Command not excecuted."));
		return;
	}
