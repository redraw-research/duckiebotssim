...
void UAdjustRenderQualityCommand::Execute() {
	UE_LOG(LogDuckiebotsSim, Log, TEXT("UAdjustRenderQualityCommand::Execute change render quality"));

	if (StringParams.size() != 0 || NumberParams.size() != 1) {
		UE_LOG(LogDuckiebotsSim, Error, TEXT("Unexpected argument length found in UAdjustRenderQualityCommand. Command not executed."));
		return;
	}
	...
	if (quality < 0 || quality > 3)	{
		UE_LOG(LogDuckiebotsSim, Warning, TEXT("Invalid quality passed to UAdjustRenderQualityCommand!"));
		return;
	}
	...
	UE_LOG(LogDuckiebotsSim, Log, TEXT("UAdjustRenderQualityCommand was successful"));
}
