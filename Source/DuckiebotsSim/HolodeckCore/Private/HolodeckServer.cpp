...
void UHolodeckServer::Start() {
    UE_LOG(LogDuckiebotsSim, Log, TEXT("Initializing HolodeckServer"));
    if (bIsRunning) {
        UE_LOG(LogDuckiebotsSim, Warning, TEXT("HolodeckServer is already running! Bringing it down and up"));
        Kill();
    }

    if (!FParse::Value(FCommandLine::Get(), TEXT("HolodeckUUID="), UUID))
        UUID = "";
    UE_LOG(LogDuckiebotsSim, Log, TEXT("UUID: %s"), *UUID);
    ...
    bIsRunning = true;
    UE_LOG(LogDuckiebotsSim, Log, TEXT("HolodeckServer started successfully"));
}

void UHolodeckServer::Kill() {
    UE_LOG(LogDuckiebotsSim, Log, TEXT("Killing HolodeckServer"));
    ...
    bIsRunning = false;
    UE_LOG(LogDuckiebotsSim, Log, TEXT("HolodeckServer successfully shut down"));
}
...
void* UHolodeckServer::Malloc(const std::string& Key, unsigned int BufferSize) {
    ...
    if (!Memory.count(Key) || Memory[Key]->Size() != BufferSize) {
        UE_LOG(LogDuckiebotsSim, Log, TEXT("Mallocing %u bytes for key %s"), BufferSize, UTF8_TO_TCHAR(Key.c_str()));
...
void UHolodeckServer::Acquire() {
    UE_LOG(LogDuckiebotsSim, VeryVerbose, TEXT("HolodeckServer Acquiring"));
#if PLATFORM_WINDOWS
...
void UHolodeckServer::Release() {
    UE_LOG(LogDuckiebotsSim, VeryVerbose, TEXT("HolodeckServer Releasing"));
#if PLATFORM_WINDOWS
...
void UHolodeckServer::LogSystemError(const std::string& errorMessage) {
    UE_LOG(LogDuckiebotsSim, Fatal, TEXT("%s - Error code: %d=%s"), ANSI_TO_TCHAR(errorMessage.c_str()), errno, ANSI_TO_TCHAR(strerror(errno)));
}
