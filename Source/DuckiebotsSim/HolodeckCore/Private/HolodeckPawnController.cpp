...
void AHolodeckPawnController::OnPossess(APawn* InPawn) {
	...
	if (ControlledAgent == nullptr) {
		UE_LOG(LogDuckiebotsSim, Fatal, TEXT("HolodeckPawnController attached to non-HolodeckAgent!"));
	}

	ControlledAgent->Controller = this;
	SetPawn(InPawn);

	UE_LOG(LogDuckiebotsSim, Log, TEXT("Pawn Possessed: %s, Controlled by: %s"), *InPawn->GetHumanReadableName(), *this->GetClass()->GetName());
	UpdateServerInfo();
...
void AHolodeckPawnController::Tick(float DeltaSeconds) {
	...
	if (Server == nullptr) {
		UE_LOG(LogDuckiebotsSim, Fatal, TEXT("HolodeckPawnController couldn't find server..."));
	}
...
void AHolodeckPawnController::UpdateServerInfo() {
	...
	if (Instance != nullptr)
		...
		UE_LOG(LogDuckiebotsSim, Warning, TEXT("Game Instance is not UHolodeckGameInstance."));
}
...
void AHolodeckPawnController::ExecuteTeleport() {
	UE_LOG(LogDuckiebotsSim, Log, TEXT("Executing teleport"));
	if (ControlledAgent == nullptr) {
		UE_LOG(LogDuckiebotsSim, Warning, TEXT("AHolodeckPawnController::ExecuteTeleport: Couldn't get reference to controlled HolodeckAgent"));
		return;
	}
...
void AHolodeckPawnController::ExecuteSetState() {
	UE_LOG(LogDuckiebotsSim, Log, TEXT("AHolodeckPawnController::ExecuteSetState"));

	if (ControlledAgent == nullptr) {
		UE_LOG(LogDuckiebotsSim, Fatal, TEXT("AHolodeckPawnController::ExecuteSetState: Couldn't get reference to controlled HolodeckAgent"));
		return;
	}
