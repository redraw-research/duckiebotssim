...
std::string GetLastErrorAsString()
{
	...
	UE_LOG(LogDuckiebotsSim, Error, TEXT("Error: %d"), errorMessageID);
...
HolodeckSharedMemory::HolodeckSharedMemory(const std::string& Name, unsigned int BufferSize, const std::string& UUID) :
	...
	UE_LOG(LogDuckiebotsSim, Log, TEXT("HolodeckSharedMemory:: Creating file mapping of size %d with path %s..."), BufferSize, ANSI_TO_TCHAR(MemPath.c_str()));

	MemFile = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, this->MemSize, WindowsMemPath);

	if (GetLastError() == ERROR_ALREADY_EXISTS) {
		UE_LOG(LogDuckiebotsSim, Warning, TEXT("HolodeckSharedMemory:: Unable to create MemFile! The path %s already exists. Hopefully it is the correct size..."), ANSI_TO_TCHAR(MemPath.c_str()));
	}

	if (!MemFile) {
		std::string msg = GetLastErrorAsString();
		UE_LOG(LogDuckiebotsSim, Fatal, TEXT("HolodeckSharedMemory:: Unable to create MemFile! %s"), ANSI_TO_TCHAR(msg.c_str()));
	}
	...
	if (!MemPointer) {
		std::string msg = GetLastErrorAsString();
		UE_LOG(LogDuckiebotsSim, Fatal, TEXT("HolodeckSharedMemory:: Unable to create MemPointer! %s"), ANSI_TO_TCHAR(msg.c_str()));
	}
...
void HolodeckSharedMemory::LogSystemError(const std::string& errorMessage) {
	...
	UE_LOG(LogDuckiebotsSim, Fatal, TEXT("%s"), ANSI_TO_TCHAR(errorMessage.c_str()));
}
